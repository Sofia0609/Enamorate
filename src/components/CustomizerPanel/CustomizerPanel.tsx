...
export const CustomizerPanel = ({ product }: Props) => {
  ...
  const toggleAdicional = (item: Additional) => {
    setCustomization(prev => {
      const exists = prev.adicionales.find(a => a.id === item.id);
      return {
        ...prev,
        adicionales: exists
          ? prev.adicionales.filter(a => a.id !== item.id)
          : [...prev.adicionales, item],
      };
    });
  };
  ...
  const buildWhatsAppMessage = () => {
    const lines: string[] = [
      `¡Hola! Quiero pedir un desayuno sorpresa`,
      ``,
      `*Desayuno:* ${product.name} (${formatPrice(product.price)})`,
    ];

    if (customization.proteina) {
      const extra = customization.proteina.extraCost > 0
        ? ` (+${formatPrice(customization.proteina.extraCost)})`
        : customization.proteina.extraCost < 0
        ? ` (${formatPrice(customization.proteina.extraCost)})`
        : ' (incluido)';
    ...
    if (isMultiSelect && bebidasSeleccionadas.length > 0) {
      ...
      const extra = customization.bebida.extraCost > 0
        ...
        ? ` (${formatPrice(customization.bebida.extraCost)})`
        : ' (incluido)';
      lines.push(`*Bebida:* ${customization.bebida.label}${extra}`);
    }

    if (customization.adicionales.length > 0) {
      lines.push(`*Adicionales:*`);
      customization.adicionales.forEach(a => {
        lines.push(`  • ${a.name}: ${formatPrice(a.price)}`);
      });
    }

    if (customization.mensaje.trim()) {
      lines.push(`*Mensaje para la tarjeta:* "${customization.mensaje.trim()}"`);
    }

    lines.push('');
    lines.push(`*Total estimado: ${formatPrice(totalPrice)}*`);
    lines.push('');
    lines.push('¿Pueden confirmarme disponibilidad y detalles del envío?');
  ...
  return (
    <div className={styles.panel}>
      ...
      <div className={styles.priceRow}>
        ...
        <span className={styles.delivery}>Envío gratis en Cali</span>
      </div>
        ...
        <div className={styles.customizer}>
          ...
          {product.customizable.proteinas.length > 0 && (
            <div className={styles.group}>
              <p className={styles.groupLabel}>Proteína</p>
              <div className={styles.options}>
                {product.customizable.proteinas.map(option => {
                  const isSelected = customization.proteina?.id === option.id;
                  return (
                    <button
                      key={option.id}
                      className={`${styles.option} ${isSelected ? styles.optionSelected : ''}`}
                      onClick={() => setCustomization(prev => ({ ...prev, proteina: option }))}
      ...
      <div className={styles.adicionalesSection}>
        <button
          className={styles.adicionalesToggle}
        ...
        {adicionalesOpen && (
          <div className={styles.adicionalesList}>
            {adicionales.map(item => {
              const isSelected = customization.adicionales.some(a => a.id === item.id);
              return (
                <button
                  key={item.id}
                  className={`${styles.adicionalItem} ${isSelected ? styles.adicionalSelected : ''}`}
                  onClick={() => toggleAdicional(item)}
                  aria-pressed={isSelected}
                >
                  <span className={styles.adicionalCheck}>{isSelected ? '✓' : '+'}</span>
                  <span className={styles.adicionalName}>
                    {item.name}
                    {item.note && <span className={styles.adicionalNote}> · {item.note}</span>}
                  </span>
                  <span className={styles.adicionalPrice}>{formatPrice(item.price)}</span>
                </button>
              );
            })}
          </div>
      ...
      {/* Mensaje tarjeta */}
      <div className={styles.mensajeGroup}>
        <label htmlFor="mensaje-tarjeta" className={styles.mensajeLabel}>
          Mensaje para la tarjeta <span className={styles.optional}>(opcional)</span>
        </label>
        <textarea
          id="mensaje-tarjeta"
          className={styles.mensajeInput}
          placeholder="Escribe aquí el mensaje especial que quieres en la tarjeta..."
          rows={3}
          maxLength={200}
          value={customization.mensaje}
          onChange={e => setCustomization(prev => ({ ...prev, mensaje: e.target.value }))}
        />
        <p className={styles.charCount}>{customization.mensaje.length}/200</p>
      </div>

      {/* Resumen */}
      {(customization.adicionales.length > 0 || extraCost !== 0) && (
        <div className={styles.summary}>
          <p className={styles.summaryTitle}>Resumen de tu pedido</p>
          <div className={styles.summaryRow}>
            <span>{product.name}</span>
            <span>{formatPrice(product.price)}</span>
          </div>
          {customization.proteina && customization.proteina.extraCost !== 0 && (
            <div className={styles.summaryRow}>
              <span>Proteína: {customization.proteina.label}</span>
      ...
      {/* CTA WhatsApp */}
      <button className={styles.whatsappBtn} onClick={handleComprar}>
